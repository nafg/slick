...
trait Session extends java.io.Closeable with Logging { self =>
  ...
  final def prepareInsertStatement(sql: String, autogeneratedColumns: Seq[String]): PreparedStatement =
    conn.prepareStatement(sql, autogeneratedColumns.toArray)
  ...
  final def withPreparedInsertStatement[T](sql: String, autogeneratedColumns: Seq[String])(f: PreparedStatement => T): T = {
    val st = prepareInsertStatement(sql, autogeneratedColumns)
